...
void main()
{
	...
	int startadd, size, addr, temp, count, byte, codesize, relocationaddr;
	size_t len;
	reset();
	printf("Relocation Address :- ");
	scanf("%x", &relocationaddr);
	...
	while (record[0] == 'T')
	{
		...
		while ((tchar = strtok(NULL, "^")) != NULL)
		{
			...
			{
				if ((strlen(tchar) == 6) && (strtol(tchar, &end, 16) - startadd) >= 0)
				{
					temp = strtol(tchar, &end, 16) - startadd + relocationaddr;
					sprintf(tchar, "%6x", temp);
				}
	...
	write_text(codesize, relocationaddr);
	printf("Execution Address :- %x\n", addr + relocationaddr - startadd);
}
